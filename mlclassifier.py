...
# Machine Learning Classifier for Password Strength
class PasswordMLClassifier:
    ...
    def _extract_features(self, password: str) -> np.ndarray:
        ...
        entropy = len(password) * math.log2(charset_size) if charset_size > 0 and len(password) > 0 else 0
        features.append(entropy)

        # 6: Contains common words
        ...
        # 9: Contains year
        has_year = 1 if re.search(r'(19\d{2}|20\d{2})', password) else 0
        features.append(has_year)

        # 10: Consecutive repeated characters
        has_repeats = 0
        ...
        # 11: Count of character types (0–4)
        types_count = sum([
    ...
    def _auto_label_password(self, password: str) -> int:
        ...
        if (length < 8 or types_count <= 1 or entropy < 40
                or has_common_word or has_repeats):
            return 0  # WEAK
        if (length >= 12 and types_count >= 3 and entropy >= 80
                and not has_sequence and not has_year and not has_common_word):
            return 2  # STRONG
        return 1  # MEDIUM
    ...
    def train(self, X_train: np.ndarray = None, y_train: np.ndarray = None):
        ...
        print(classification_report(y_val, y_pred, target_names=['WEAK', 'MEDIUM', 'STRONG']))
        ...
        try:
            ...
            for i in range(min(5, len(indices))):
                ...
                print(f"  {i+1}. {name:20s}: {importances[idx]:.3f}")
            print()
        except Exception as e:
    ...
    def predict(self, password: str) -> Tuple[str, float]:
        ...
        return self.label_mapping[prediction], float(confidence)
